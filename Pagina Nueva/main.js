...
function deleteRow(model, id) {
	if (confirm("Esta seguro que desea eliminar esta entrada?")) {
		$.ajax({type:'POST', url: PATH+model+'/destroy.php', data: "id="+id, success:
				function(response) {
					$('#main').html(response);
				}
			});
...
function deleteRowSpecial(model, id, params) {
	if (confirm("Esta seguro que desea eliminar esta entrada?")) {
		$.ajax({type:'POST', url: PATH+model+'/destroy.php', data: "id="+id+"&"+params, success:
               function(response) {
               $('#main').html(response);
               }
               });
...
function filter(search, model) {
	$.ajax({type:'POST', url: PATH+model+'/search.php', data: "search="+search, success:
		function(response) {
...
function filterSpecial(search, model, params) {
	$.ajax({type:'POST', url: PATH+model+'/search.php', data: "search="+search+"&"+params, success:
           function(response) {
           $('tbody#'+model+'-content').html(response);
           }
           });
...
function search(search, model) {
	$.ajax({type:'POST', url: PATH+model+'/resource_search.php', data: "search="+search, success:
		function(response) {
...
function returnLoan(model, id) {
	if (confirm("Esta seguro que desea marcar como entregado?")) {
		$.ajax({type:'POST', url: PATH+model+'/return.php', data: "id="+id, success:
				function(response) {
					$('#main').html(response);
				}
			});
