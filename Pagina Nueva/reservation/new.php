...
// Abajo esta la forma para llenado de una nueva reservacion
?>
<h2>Reservar Laboratorio</h2>
<form id='new-reservation'>
<div class="label">Usuario: <?=$user['username']?></div>
<div class="control-lab">
<div class="label">Laboratorios disponibles:</div>
<?	foreach($labs as $lab) { ?>
		<input type="radio" onclick="desplegarHorarios(this.value)" name="LabId" value="<?=$lab['id']?>"><?=$lab['room']?><br />
<? 	} ?>
</div>

<input type="hidden" id="fechaReservacion" name="fechaReservacion" value="">
...
<a class="btn btn-primary" onClick="submitForm('reservation/create.php','new-reservation','new-reservation');">Reservar</a>
<a href="<?=$path2?>reservations" class="btn">Cancelar</a>
</div>
</form>

<script>
function desplegarHorarios(labId){
	$.ajax({
			   url:'reservation/desplegar.php',
			   data: { "labId" : labId },
...
function retro(element){
	$.ajax({
		   url:'reservation/retro.php',
		   data: { "date" : element.getAttribute("data-date"),
